...
function createReportBuilder({ getLlm }) {
  async function generateReportMarkdown(prompt, options = {}) {
    const llm = getLlm(options.req);
    ...
    for (let attempt = 1; attempt <= 2 && shouldContinueReport(markdown); attempt += 1) {
      ...
      const continuation = await getLlm(options.req).callText(buildReportContinuationPrompt(markdown), 0.35, false, {
