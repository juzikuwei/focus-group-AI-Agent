  ...
  async function fetchWithRetry(url, options, meta = {}) {
    ...
    for (let attempt = 1; attempt <= 4; attempt += 1) {
      try {
        ...
        if (attempt < 4) {
          console.warn(`[AI retry] ${meta.label || "request"} network error on attempt ${attempt}: ${error.message}; retrying...`);
      ...
      console.warn(`[AI retry] ${meta.label || "request"} got ${response.status} on attempt ${attempt}; waiting ${2500 * attempt}ms.`);
      await wait(2500 * attempt);
    }
  ...
  async function fetchStreamingWithRetry(url, options, meta = {}) {
    ...
    for (let attempt = 1; attempt <= 4; attempt += 1) {
      ...
      try {
        ...
        if (error.name === "AbortError") throw error;
        if (attempt < 4) {
          console.warn(`[AI retry] ${meta.label || "request"} stream network error on attempt ${attempt}: ${error.message}; retrying...`);
          await wait(2500 * attempt);
          continue;
        }
        if (error.name === "TimeoutError") {
...
function createLlmClientWithOverrides(defaultProvider, userSettings) {
  if (!userSettings || !userSettings.apiKey) {
    return null; // No override, use default
  }
  const providerName = normalizeProviderName(userSettings.apiProvider || defaultProvider.name);
  const format = defaultProvider.format || getProviderFormat(providerName);
  const provider = {
    name: providerName || defaultProvider.name,
    format,
    endpoint: normalizeProviderEndpoint(userSettings.apiBaseUrl || defaultProvider.endpoint, format),
    model: userSettings.model || defaultProvider.model,
    apiKey: userSettings.apiKey,
    requiresKey: true,
    supportsJsonMode: defaultProvider.supportsJsonMode !== false,
  };
  return createLlmClient({ activeProvider: provider, activeProviderName: provider.name });
}

function getProviderFormat(providerName) {
  const formats = { anthropic: "anthropic", gemini: "gemini" };
  return formats[normalizeProviderName(providerName)] || "openai";
}

function normalizeProviderName(name) {
  const key = String(name || "").trim();
  return key === "mino" ? "mimo" : key;
}

function normalizeProviderEndpoint(endpoint, format) {
  ...
  if (format === "gemini") {
    ...
    if (/\/v\d+(?:beta)?$/i.test(clean)) return `${clean}/models`;
    return `${clean}/v1beta/models`;
  }
