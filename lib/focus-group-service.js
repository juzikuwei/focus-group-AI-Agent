...
const { toStreamErrorEvent } = require("./error-response");
const {
...
function createFocusGroupService({ promptStore, llm, searchClient }) {
  ...
  const reportBuilder = createReportBuilder({ getLlm });

  function getLlm(req) {
    return req?.userLlm || llm;
  }

  function getSearchClient(req) {
    return req?.userSearchClient || searchClient;
  }
  ...
  async function handlePersonas(req, res, sendJson) {
    ...
    getLlm(req).assertProviderReady();

    const prompt = renderPrompt("persona-generator.md", {
      ...config,
      ...buildInterviewControls(config),
    });
    const data = await getLlm(req).callJson(prompt, 0.65, {
      label: "personas",
  ...
  async function handleModeratorGuide(req, res, sendJson) {
    ...
    const data = await getLlm(req).callJson(prompt, 0.45, {
  ...
  async function handleSessionRoundStream(req, res) {
    ...
    validateSessionRoundStreamPayload(payload);
    const { config, personas, topics, topic, roundNumber, priorMessages, moderatorGuide, participantStates, contextState } = payload;
    const signal = getRequestSignal(req);
    getLlm(req).assertProviderReady();
  ...
  async function handleReportStream(req, res) {
    const payload = await readJson(req);
    validateReportStreamPayload(payload);
    const { config, personas, messages, moderatorGuide, participantStates, contextState, evidencePack } = payload;
    const signal = getRequestSignal(req);
  ...
  function buildReportPrompt({ config, personas, messages, moderatorGuide, participantStates, contextState, evidencePack, req }) {
    requireProjectConfig(config);
    requireArray(personas, "personas", { minLength: 1 });
  ...
  async function handleQuickFill(req, res, sendJson) {
    ...
    const searchResearch = await buildQuickFillResearch(seedText, signal, req);
    const prompt = renderPrompt("quick-fill.md", {
      seed: seedText,
      searchResearchJson: compactJson(searchResearch),
    });
    const data = await getLlm(req).callJson(prompt, 0.7, {
  ...
  async function generateDeepSessionRound({ config, personas, topic, roundNumber, priorMessages, moderatorGuide, participantStates, contextState, signal, req, onEvent }) {
  ...
  async function decideRoundContinuation({ shared, personas, messages, roundNumber, topic, priorContext, completedWaves, maxWaves, signal }) {
    ...
    const data = await getLlm(shared.req).callJson(prompt, 0.35, {
  ...
  async function generateModeratorTurn(values, personas) {
    ...
    const data = await getLlm(values.req).callJson(prompt, 0.62, {
      label: values.label,
  ...
  async function generateParticipantGroupTurn(values, allPersonas, selectedPersonas) {
    ...
    const data = await getLlm(values.req).callJson(prompt, 0.8, {
      label: values.label,
  ...
  async function updateParticipantStates({ config, personas, moderatorGuide, participantStates, roundNumber, topic, roundMessages, signal, req }) {
    ...
    const data = await getLlm(req).callJson(prompt, 0.35, {
  ...
  async function buildEvidencePackForSession({ config, topics, personas, moderatorGuide, signal, req }) {
    const activeSearchClient = getSearchClient(req);
    if (!activeSearchClient?.canSearch?.()) {
    ...
    getLlm(req).assertProviderReady();
    try {
      const maxQueries = activeSearchClient.getStatus().maxQueries || 4;
      let queries = [];
      try {
        ...
        const plan = await getLlm(req).callJson(planPrompt, 0.35, {
          label: "search.plan",
          maxTokens: 1400,
      ...
      const searchResults = await activeSearchClient.searchMany(queries, { signal });
      ...
      const data = await getLlm(req).callJson(packPrompt, 0.28, {
        label: "evidence.pack",
        maxTokens: 5200,
        ...
        if (error.name === "AbortError") throw error;
        console.warn(`[search evidence] model synthesis failed; using direct search fallback: ${error.message}`);
        return {
          evidencePack: buildSearchFallbackEvidencePack({
            config,
            searchResults,
            queries,
            error,
  ...
  function buildSearchFallbackEvidencePack({ config, searchResults, queries, error }) {
    const prepared = prepareSearchResultsForPrompt(searchResults);
    const sourceCards = [];
    prepared.results.forEach((group) => {
      (group.results || []).forEach((result) => {
        if (sourceCards.length >= 8) return;
        const keyFacts = dedupeShortList([group.answer, result.content], 3, 170);
        sourceCards.push({
          id: `S${sourceCards.length + 1}`,
          type: group.type || "other",
          title: result.title || group.query || `来源 ${sourceCards.length + 1}`,
          url: result.url || "",
          sourceDate: result.publishedDate || "未知",
          reliability: Number(result.score) >= 0.75 ? "medium" : "low",
          keyFacts,
          userSignals: dedupeShortList([result.content], 2, 150),
          competitors: [],
          relevantFor: dedupeShortList([group.purpose, group.query], 3, 80),
          quoteSnippets: dedupeShortList([result.content], 1, 120),
        });
      });
    });

    const errorText = String(error?.safeMessage || error?.message || error || "").slice(0, 180);
    return normalizeEvidencePack({
      status: sourceCards.length ? "used" : "failed",
      generatedAt: todayIsoDate(),
      topic: config.productConcept || config.projectName || "未命名研究",
      stimulusScript: sourceCards.length
        ? "我们整理了一些公开网页、产品页和评论片段，供大家讨论时参考。"
        : "",
      error: sourceCards.length ? "AI 资料包 JSON 解析失败，已改用搜索结果摘要。" : errorText,
      queries,
      sourceCards,
      marketPatterns: dedupeShortList(prepared.results.map((group) => group.answer), 5, 170),
      competitors: [],
      commonComplaints: [],
      purchaseBarriers: [],
      openQuestions: dedupeShortList(queries.map((item) => `访谈中验证：${item.purpose || item.query}`), 6, 150),
    });
  }

  function dedupeShortList(values, limit, maxLength) {
    const seen = new Set();
    const result = [];
    (values || []).forEach((value) => {
      const text = truncateText(cleanGeneratedText(value), maxLength);
      if (!text) return;
  ...
  async function buildQuickFillResearch(seed, signal, req) {
    const activeSearchClient = getSearchClient(req);
    if (!activeSearchClient?.canSearch?.()) {
    ...
    try {
      const maxQueries = Math.min(4, activeSearchClient.getStatus().maxQueries || 4);
      let queries = [];
      try {
        const planPrompt = renderPrompt("quick-fill-search-plan.md", { seed, maxQueries });
        const plan = await getLlm(req).callJson(planPrompt, 0.3, {
...
function writeStreamError(res, error) {
  ...
  if (event.status >= 500) console.error(error);
  writeStreamEvent(res, event);
}

function validateSessionRoundStreamPayload(payload) {
  requirePlainObject(payload, "body");
  requireProjectConfig(payload.config);
  requireObjectArray(payload.personas, "personas", {
    minLength: 1,
    requiredStringFields: ["name"],
  });
  requireNonEmptyString(payload.topic, "topic");
  requirePositiveInteger(payload.roundNumber, "roundNumber");
  if (payload.topics !== undefined) requireStringArray(payload.topics, "topics", { minLength: 1 });
  if (payload.priorMessages !== undefined) requireMessageArray(payload.priorMessages, "priorMessages");
  if (payload.participantStates !== undefined && payload.participantStates !== null) {
    requireObjectArray(payload.participantStates, "participantStates");
  }
  requireOptionalPlainObject(payload.moderatorGuide, "moderatorGuide");
  requireOptionalPlainObject(payload.contextState, "contextState");
}

function validateReportStreamPayload(payload) {
  requirePlainObject(payload, "body");
  requireProjectConfig(payload.config);
  requireObjectArray(payload.personas, "personas", {
    minLength: 1,
  ...
  requireMessageArray(payload.messages, "messages", { minLength: 1 });
  if (payload.participantStates !== undefined && payload.participantStates !== null) {
    requireObjectArray(payload.participantStates, "participantStates");
  }
  requireOptionalPlainObject(payload.moderatorGuide, "moderatorGuide");
  requireOptionalPlainObject(payload.contextState, "contextState");
  requireOptionalPlainObject(payload.evidencePack, "evidencePack");
}
