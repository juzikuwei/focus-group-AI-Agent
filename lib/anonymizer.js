...
function buildAnonymizedReportContext({ config, personas, messages, moderatorGuide, participantStates, contextState, buildReportContextState }) {
  ...
  const anonymousPersonas = personas.map((persona, index) => {
    ...
    return [
      ...
      persona.snapshot ? anonymizeText(persona.snapshot, aliasByName) : "",
      persona.currentAlternative ? `现替代：${anonymizeText(persona.currentAlternative, aliasByName)}` : "",
      persona.switchTrigger ? `触发：${anonymizeText(persona.switchTrigger, aliasByName)}` : "",
      persona.budgetAnchor ? `价格锚点：${anonymizeText(persona.budgetAnchor, aliasByName)}` : "",
      persona.evidenceNeeded ? `证据门槛：${anonymizeText(persona.evidenceNeeded, aliasByName)}` : "",
      persona.discussionRole ? `讨论角色：${anonymizeText(persona.discussionRole, aliasByName)}` : "",
    ]
      .filter(Boolean)
      .join("，");
  });
